...
class TestClampAndNormalize(TorchScriptTestMixin, TorchScriptTraceTestMixin):
    ...
    @pytest.mark.parametrize(
        "inputs,min,max,expected",
        [
            pytest.param(3, -10, 10, 0.65),
            pytest.param(0, -10, 10, 0.5),
            pytest.param(2, 0, 10, 0.2),
            pytest.param(0, -10, 0, 1.0),
        ],
    )
    def test_result(self, inputs, min, max, expected):
        inputs = torch.tensor([inputs])
        inputs_copy = inputs.clone()
        expected = torch.tensor([expected])
        layer = ClampAndNormalize(min, max)
        actual = layer(inputs)
