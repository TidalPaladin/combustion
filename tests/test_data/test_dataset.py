...
from combustion.data.dataset import SerializeMixin
...
def file_factory(tmp_path):
    def func(filename, subdir=None):
        if subdir is not None and not os.path.isdir(subdir):
            os.makedirs(subdir)
...
@pytest.fixture
def setup_input_files(file_factory, subdirs, ext):
    for subdir in subdirs:
        files = ["foo.txt", "bar.pth", "baz.png", "file.csv"]
        for f in files:
            file_factory(f, subdir)


@pytest.mark.usefixtures("setup_input_files")
@pytest.mark.parametrize("subdirs", [pytest.param(["."], id="flat"), pytest.param(["dir1", "dir2"], id="nested"),])
@pytest.mark.parametrize("ext", [".pth", ".csv", ".png",])
def test_matches_glob(subdirs, ext):
    pattern = f"*{ext}"
    dataset = ...
    for target in dataset.files:
        assert ext in target
